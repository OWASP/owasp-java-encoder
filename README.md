...
* change: the JavaScript encoders (`forJavaScript`, `forJavaScriptAttribute`, `forJavaScriptBlock`, `forJavaScriptSource`) now hex encode U+007F (DEL) and the C1 control characters U+0080 to U+009F as `\xNN` instead of passing them through, for consistency with the other encoders. This changes output for inputs containing these characters but not the decoded string value; it is not a security fix, since JavaScript does not treat them as line terminators [#163](https://github.com/OWASP/owasp-java-encoder/issues/163).
